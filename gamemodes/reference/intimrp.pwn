...
#define         SERVER_VERSION				"IM:RP v3.09b"
#define         SERVER_MAP					"Las Venturas"
#define         SERVER_SITE					"www.im-rp.net"
...
forward PayDay();
forward MySQL_Change_Name(playerid, newname[], adminid, connectionHandle);
...
forward MySQL_Player_Referral(playerid, referer[], connectionHandle);
forward MySQL_Unban_Player(name[], adminid, connectionHandle);
forward MySQL_Suspend_Player(name[], adminid, connectionHandle);
forward MySQL_Unban_Ip(ip[], adminid, connectionHandle);
forward MySQL_Faction_Load(connectionHandle);
forward MySQL_Business_Load(connectionHandle);
forward MySQL_Vault_Load(connectionHandle);
forward MySQL_Vehicle_Load(connectionHandle);
...
#define LOCATION_MINEPROCESSING		-212.2387,2121.3508,-14.5078
...
{
	AntiDeAMX();
	SendRconCommand("hostname [BETA PHASE II] "SERVER_NAME);
...
public OnGameModeInit()
{
	...
	CreateDynamic3DTextLabel("Weapon Factory\nType /getgun to produce a gun", COLOR_YELLOW, LOCATION_WEAPONFACTORY, 8.0);
	CreateDynamic3DTextLabel("Mine Spot\nType /mine to start mining", COLOR_YELLOW, LOCATION_MINESPOT, 8.0);
...
*/
	...
	Login5 = TextDrawCreate(279.000091, 9.125836, "WWW.IM-RP.NET");
	...
	Login8 = TextDrawCreate(242.333312, 393.244415, "TS3: ts.im-rp.net");
...
stock MySQL_Initialize()
{
	...
	mConnectionHandle = mysql_connect(MYSQL_HOST, MYSQL_USERNAME, MYSQL_DATABASE, MYSQL_PASSWORD, 3306, true);
...
public OnPlayerConnect(playerid)
{
	...
	SetPVarInt(playerid, "pSafeMode", 1);
	...
	DeletePVar(playerid, "pGodMode"); DeletePVar(playerid, "pPreGodHealth"); DeletePVar(playerid, "pPreGodArmor");
...
{
	ShowModelSelectionMenu2(playerid, buyable, "Vehicles");
	...
	switch (resultid)
	{
	    case THREAD_IP_LOOKUP:
	    {
	        ...
	        if(rowCount > 0)
	        {
				...
				cache_get_field_content(0, "Username", bUsername, connectionHandle, MAX_PLAYER_NAME);
				cache_get_field_content(0, "Reason", bReason, connectionHandle, 64);
	    ...
	    case THREAD_ACCOUNT_LOOKUP:
	    {
	        ...
	        {
	            cache_get_row_count(connectionHandle);
                cache_get_field_content(0, "Password", PlayerInfo[extraid][pPassword], connectionHandle, 129);
               	LoadPlayerAccount(extraid, connectionHandle);
	        	if(!IsRPName(extraid) && !IsAdmin(extraid, ADMIN_LEVEL_TWO))
				{
				    SendClientMessageEx(extraid, COLOR_YELLOW, "Your administrator account has been suspended. Please contact a human resources representative for more information.");
				    KickEx(extraid);
				}
	    ...
	    {
	        new szQuery[128];
	        PlayerInfo[extraid][pId] = cache_insert_id(mConnectionHandle);
			format(szQuery, sizeof(szQuery), "SELECT * FROM `accounts` WHERE `Username` = '%s' LIMIT 0, 1", PlayerInfo[extraid][pUsername]);
	...
	switch(dialogid)
	{
		...
		case DIALOG_REFERER:
		{
		    if(response)
		    {
				...
				mysql_real_escape_string(inputtext, rname, mConnectionHandle);
                mysql_format(mConnectionHandle, string, sizeof(string), "SELECT * FROM `accounts` WHERE `Username` = '%s'", rname);
				mysql_function_query(mConnectionHandle, string, true, "MySQL_Player_Referral", "isi", playerid, inputtext, mConnectionHandle);
		    }
		    else
		    {
		    	for(new i = 0; i < 20; i++) SendClientMessageEx(playerid, COLOR_WHITE, "");
		...
		case DIALOG_WAYPOINT_SUBMIT:
		{
		    ...
		    switch (WaypointEditOption[playerid])
		    {
		        case 1:
		        {
		            ...
		            new
		                wEscapedText[65];
					mysql_real_escape_string(inputtext, wEscapedText, mConnectionHandle);
		        ...
		        case 19:
		        {
					...
					new waypointPass[9];
					mysql_real_escape_string(inputtext, waypointPass, mConnectionHandle);
		...
		case DIALOG_BUSINESS_SUBMIT:
		{
		    ...
		    switch (BusinessEditOption[playerid])
		    {
		        case 1:
		        {
		        	...
		        	new bizName[17];
             	 	mysql_real_escape_string(inputtext, bizName, mConnectionHandle);
		...
		case DIALOG_FACTION_ADMIN_EDIT:
		{
		    ...
		    switch (FactionEditOption[playerid])
		    {
		        case 1:
		        {
		        	...
		        	new factionName[25];
             	 	mysql_real_escape_string(inputtext, factionName, mConnectionHandle);
		        ...
		        case 2:
		        {
		        	...
		        	new factionMotd[97];
             	 	mysql_real_escape_string(inputtext, factionMotd, mConnectionHandle);
		...
		case DIALOG_FACTION_ADMIN_DIV:
		{
		    ...
		    switch(FactionEditOption[playerid])
		    {
				case 0:
		        {
		        	...
		        	new fDiv[9];
             	 	mysql_real_escape_string(inputtext, fDiv, mConnectionHandle);
		...
		case DIALOG_FACTION_ADMIN_RANK:
		{
		    ...
		    switch(FactionEditOption[playerid])
		    {
				case 0:
		        {
		        	...
		        	new factionRank[33];
             	 	mysql_real_escape_string(inputtext, factionRank, mConnectionHandle);
		...
		case DIALOG_CITY_HALL_NC:
		{
			...
			new string[160];
			mysql_real_escape_string(inputtext, string, mConnectionHandle);
...
public MySQL_Change_Name(playerid, newname[], adminid, connectionHandle)
{
...
CMD:approvename(playerid, params[])
{
	if(IsAdmin(playerid, ADMIN_LEVEL_THREE))
	{
		...
		mysql_function_query(mConnectionHandle, string, true, "MySQL_Change_Name", "isii", targetid, pName, playerid, mConnectionHandle);
	}
	return 1;
}
...
{
	SendClientMessageEx(playerid, COLOR_GREEN, "Server Information{FFFFFF} | {33AA33}SA-MP:{FFFFFF} samp.im-rp.net:7777 | {33AA33}Website:{FFFFFF} im-rp.net | {33AA33}TS3:{FFFFFF} ts.im-rp.net");
	return 1;
}
...
public Heartbeat()
{
	foreach(new i: Player)
	{
	    if(GetPVarInt(i, "gPlayerLogged") == 1 && IsPlayerTabbed(i) == false && PlayerInfo[i][pTutorial] == 1)
	    {
			...
			if(!IsAdmin(i, ADMIN_LEVEL_TWO) && GetPVarType(i, "pSafeMode") == 0 && GetPlayerState(i) != PLAYER_STATE_WASTED && GetPlayerState(i) == PLAYER_STATE_SPAWNED)
			{
			    new string[128];
			    // Anti Weapon Hacks
				new weaponData[2];
...
CMD:adminhelp(playerid, params[])
{
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "As a {FFFF00}Server Moderator{FFFFFF}, you are to help players on /n(ewbie) and follow the server rules.");
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "{00FF00}Junior Admin:{FFFFFF} /a, /reports, /ar, /tr, /cr, /kick, /jail, /mute, /ban, /slap, /freeze, /unfreeze, /sendto");
		...
		SendClientMessageEx(playerid, COLOR_WHITE, "{00FF00}Junior Admin:{FFFFFF} /spectate, /gotov, /sobeitplayers, /tabbed, /revive, /god, /ooc, /check, /release");
	}
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "{FF0000}Head Admin:{FFFFFF} /addwaypoint, /editwaypoint, /addbusiness, /editbusiness");
		SendClientMessageEx(playerid, COLOR_WHITE, "{FF0000}Head Admin:{FFFFFF} /makeadmin, /givecash, /makeleader, /fackick, /givegun");
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "{0F80C8}Lead Admin:{FFFFFF} /restart, /payday, /makeadmin, /setstat, /saveaccounts");
	}
	SendClientMessageEx(playerid, COLOR_ORANGE, "-------------------------------------------------------------------");
	return 1;
}
...
CMD:god(playerid, params[])
{
    if(IsAdmin(playerid, ADMIN_LEVEL_TWO))
	{
	    if(GetPVarInt(playerid, "pGodMode") == 1)
	    {
			SetPlayerHealth(playerid, PlayerInfo[playerid][pHealth]);
			SetPlayerArmour(playerid, PlayerInfo[playerid][pArmour]);
			DeletePVar(playerid, "pGodMode");
			SendClientMessageEx(playerid, COLOR_WHITE, "God mode disabled.");
		}
		else
		{
		    ...
		    SendClientMessageEx(playerid, COLOR_WHITE, "God mode enabled.");
		}
    }
	return 1;
}
...
CMD:car(playerid, params[])
{
    ...
    {
    	return SendClientMessageEx(playerid, COLOR_GREY, "Usage: /car [engine, lights, refuel, hood, trunk, status, radio]");
    }
	...
	else if(IsAPizzaBike(vehicleid) || IsADmvCar(vehicleid) || IsATruckerVehicle(vehicleid) || (WhoRentsVehicle(vehicleid) == INVALID_PLAYER_ID && IsARentableVehicle(vehicleid)) || !IsRunnableEngine(vehicleid)) return SendClientMessageEx(playerid, COLOR_GREY, "You cannot use this command on this vehicle.");
	...
	else if(!strcmp(params, "lights"))
	{
		...
		if(VehicleInfo[vehicleid][vLights])
		{
			...
			SendClientMessageEx(playerid, COLOR_WHITE, "You have successfully turned on the lights of your vehicle.");
		}
		SetVehicleParamsEx(vehicleid, VehicleInfo[vehicleid][vEngine], VehicleInfo[vehicleid][vLights], VehicleInfo[vehicleid][vAlarm], VehicleInfo[vehicleid][vDoors], VehicleInfo[vehicleid][vBonnet], VehicleInfo[vehicleid][vBoot], VehicleInfo[vehicleid][vObjective]);
	}
...
CMD:advert(playerid, params[])
{
	...
	format(string, sizeof(string), "[IGTV-ADVERT] %s - contact %s (%d)", params, GetPlayerNameEx(playerid),PlayerInfo[playerid][pPhoneNumber]);
...
CMD:admin(playerid, params[])
{
	if(IsAdmin(playerid, ADMIN_LEVEL_TWO))
	{
		...
		new string[128];
		format(string, sizeof(string), "** %s %s: %s **", GetAdminRank(playerid), GetPlayerNameEx(playerid), params);
		foreach(new i: Player) if(IsAdmin(i, ADMIN_LEVEL_TWO)) SendClientMessageEx(i, COLOR_YELLOW, string);
	}
	return 1;
}
...
{
	new string[128];
	if(sscanf(params, "s[128]", string)) return SendClientMessageEx(playerid, COLOR_GREY, "Usage: /n(ewbie) [message]");
...
CMD:debug(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_YELLOW, string);
	format(string, sizeof(string), "Interior: %i, Virtual World: %i, SafeMode: %i", GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), GetPVarType(playerid, "pSafeMode"));
	SendClientMessageEx(playerid, COLOR_YELLOW, string);
...
public MySQL_Suspend_Player(name[], adminid, connectionHandle)
{
	new szQuery[128], rowCount, fieldCount;
	cache_get_data(rowCount, fieldCount, connectionHandle);
	if(rowCount > 0)
...
CMD:suspend(playerid, params[])
{
	...
	if(targetid != INVALID_PLAYER_ID)
	{
	    ...
	    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szQuery);
	   	...
	   	Log("logs/makeadmin.log", szQuery);
	...
	else
	{
		...
		mysql_function_query(mConnectionHandle, szQuery, true, "MySQL_Suspend_Player", "sii", pName, playerid, mConnectionHandle);
	}
	return 1;
}
...
public MySQL_Offline_Ban(name[], reason[], days, adminid, connectionHandle)
{
	...
	if(rowCount > 0)
	{
		cache_get_field_content(0, "IpAddress", szIpAddress, connectionHandle, 16);
...
CMD:makeadmin(playerid, params[])
{
    ...
    {
        new targetid, level, string[128];
		if(sscanf(params, "ui", targetid, level)) return SendClientMessageEx(playerid, COLOR_GREY, "Usage: /makeadmin [playerid] [0-5]");
		else if(!IsPlayerConnectedEx(targetid)) return SendClientMessageEx(playerid, COLOR_GREY, "Invalid player specified.");
...
CMD:gps(playerid, params[])
{
	...
	format(string, sizeof(string), "A checkpoint has been set to the specified location (%s).", GetPlayerNameEx(playerid), params);
	SendClientMessageEx(playerid, COLOR_WHITE, string);
	return 1;
}
...
CMD:goto(playerid, params[])
{
    if(IsAdmin(playerid, ADMIN_LEVEL_TWO))
	{
		...
		SendClientMessageEx(playerid, COLOR_WHITE, "You have been teleported by an administrator.");
...
public MySQL_Player_Referral(playerid, referer[], connectionHandle)
{
	new string[128], rowCount, fieldCount;
	cache_get_data(rowCount, fieldCount, connectionHandle);
	if(rowCount > 0)
	{
		for(new i = 0; i < 20; i++)
...
CMD:mine(playerid, params[])
{
    ...
    else if(IsPlayerInAnyVehicle(playerid)) return SendClientMessageEx(playerid, COLOR_WHITE, "You must be on foot to use this command.");
    else if(!IsPlayerInRangeOfPoint(playerid, 25.0, LOCATION_MINESPOT)) return SendClientMessageEx(playerid, COLOR_WHITE, "You must be near the mines to use this command.");
...
public StartMining(playerid)
{
    ...
    SetPlayerCheckpoint(playerid, LOCATION_MINEPROCESSING, 2.0);
	return 1;
}
...
CMD:admins(playerid, params[])
{
	...
	foreach (Player, i)
	{
		if(IsAdmin(i, ADMIN_LEVEL_TWO))
	    {
		    if(IsAdmin(playerid, ADMIN_LEVEL_FOUR)) format(string, sizeof(string), "%s %s (Completed Reports: %d)", GetCAdminRank(i), GetPlayerNameEx(i), PlayerInfo[i][pReports]);
			else format(string, sizeof(string), "%s %s", GetCAdminRank(i), GetPlayerNameEx(i));
		    SendClientMessageEx(playerid, COLOR_WHITE, string);
	    }
	}
	SendClientMessageEx(playerid, COLOR_ORANGE, "---------------------------------");
...
CMD:mods(playerid, params[])
{
	...
	if(IsAdmin(playerid, ADMIN_LEVEL_TWO))
	{
		...
		foreach (Player, i)
		{
			if(IsAdmin(i, ADMIN_LEVEL_ONE) && !IsAdmin(i, ADMIN_LEVEL_TWO))
		    {
...
stock ShowPlayerStats(playerid, targetid)
{
	...
	format(string, sizeof(string), "Playing Hours: %i | Experience: %i/%d | Referrer: %s | Channel :%i ", PlayerInfo[targetid][pPlayingHours], PlayerInfo[targetid][pExperience], (PlayerInfo[targetid][pLevel] + 1) * 5, GetPlayerReferer(targetid),  PlayerInfo[targetid][pChannel]);
	SendClientMessageEx(playerid, COLOR_FADE1, string);
...
stock CreateCorpse(playerid)
{
    ...
    CorpseInfo[1][cHandleId] = CreateDynamicObject(3092, cPosX, cPosY, cPosZ-0.8, -90, 0, 0);
...
stock BagCorpse(corpseid)
{
	new string[128],
    CorpseInfo[1][cBagHandleId] = CreateDynamicObject(2919, cPosX, cPosY, cPosZ-0.8, -90, 0, 0);
	return 1;
}
...
CMD:updates(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "* Added Miner job.");
	SendClientMessageEx(playerid, COLOR_WHITE, "* Brand new realistic death system.");
...
public OnPlayerDisconnect(playerid, reason)
{
	if(GetPVarInt(playerid, "gPlayerLogged") == 1)
	{
		...
		if(GetPVarType(playerid, "pSafeMode") == 1) DeletePVar(playerid, "pSafeMode");
...
public OnPlayerSpawn(playerid)
{
	if(GetPVarInt(playerid, "gPlayerLogged") == 1)
	{
		...
		if(PlayerInfo[playerid][pAdminLevel] > 0 || PlayerInfo[playerid][pLevel] < 5)
		{
			PlayerInfo[playerid][pTogNewbie] = 1;
		}
		...
		if(PlayerInfo[playerid][pWounded] != PLAYER_ALIVE)
		{
		    ...
		    SetPlayerHealthEx(playerid, 25);
  			if(GetPVarType(playerid, "pSafeMode") == 1 && PlayerInfo[playerid][pPositionX] == 0 && PlayerInfo[playerid][pPositionY] == 0 && PlayerInfo[playerid][pPositionZ] == 0)
			{
			    SendClientMessageEx(playerid, COLOR_WHITE, "It appears that you have logged out while being wounded or dead, thus ending up in the hospital.");
	  			GivePlayerDeath(playerid);
	  			DeletePVar(playerid, "pSafeMode");
			  	return 1;
	  		}
		    if(GetPVarType(playerid, "pSafeMode") == 1)
			{
				if(PlayerInfo[playerid][pWounded] == PLAYER_WOUNDED) SendClientMessageEx(playerid, COLOR_RED, "You are brutally wounded. You can now /call 911 ems or /accept death.");
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(PlayerInfo[playerid][pWounded] == PLAYER_ALIVE) MakePlayerWounded(playerid);
	else if(PlayerInfo[playerid][pWounded] == PLAYER_WOUNDED) MakePlayerDead(playerid);
	if(GetPVarInt(playerid, "BackupRequested") == 1)
	{
		DeletePVar(playerid, "BackupRequested");
  		SetPlayerToTeamColor(playerid);
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	else if(GetPVarInt(playerid, "RouteId") != 0)
	{
 	...
 	{
	 		new salary = random(50)+51, string[128];
		    SendClientMessageEx(playerid, COLOR_ORANGE, "MISSION:{FFFFFF} You have succesfully completed the mission.");
		    format(string, sizeof(string), "SALARY:{FFFFFF} $%d had been added to your paycheck for the completion of the mission.", salary);
		    SendClientMessageEx(playerid, COLOR_ORANGE, string);
		    PlayerInfo[playerid][pPaycheck] += salary;
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	if(IsPlayerRestricted(playerid) || GetPVarInt(playerid, "pGodMode")) return 0;
    RemovePlayerColorFade(playerid);
...
{
    if(GetPVarInt(hitid, "pGodMode")) return 0;
...
stock UnbanPlayerEx(name[])
{
	...
	mysql_tqueryof(szQuery, THREAD_LOG_RESULT, INVALID_PLAYER_ID, mConnectionHandle);
//	return 1;
	if(cache_affected_rows(mConnectionHandle) > 0)
	{
		format(szQuery, sizeof(szQuery), "UPDATE `accounts` SET `Banned` = '0' WHERE `Username` = '%s'", name);
		mysql_tqueryof(szQuery, THREAD_LOG_RESULT, INVALID_PLAYER_ID, mConnectionHandle);
		return 1;
...
public MySQL_Faction_Load(connectionHandle)
{
    ...
    {
        cache_get_field_content(i, "Id", szValue, mConnectionHandle); id = strval(szValue);
        cache_get_field_content(i, "Name", FactionInfo[id][fName], mConnectionHandle, MAX_PLAYER_NAME);
        cache_get_field_content(i, "Type", szValue, mConnectionHandle); FactionInfo[id][fType] = strval(szValue);
        cache_get_field_content(i, "Motd", FactionInfo[id][fMotd], mConnectionHandle, 128);
        cache_get_field_content(i, "Rank1", FactionInfo[id][fRank1], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank2", FactionInfo[id][fRank2], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank3", FactionInfo[id][fRank3], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank4", FactionInfo[id][fRank4], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank5", FactionInfo[id][fRank5], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank6", FactionInfo[id][fRank6], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank7", FactionInfo[id][fRank7], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank8", FactionInfo[id][fRank8], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank9", FactionInfo[id][fRank9], mConnectionHandle, 32);
        cache_get_field_content(i, "Rank10", FactionInfo[id][fRank10], mConnectionHandle, 32);
        cache_get_field_content(i, "Div1", FactionInfo[id][fDiv1], mConnectionHandle, 8);
        cache_get_field_content(i, "Div2", FactionInfo[id][fDiv2], mConnectionHandle, 8);
        cache_get_field_content(i, "Div3", FactionInfo[id][fDiv3], mConnectionHandle, 8);
        cache_get_field_content(i, "Div4", FactionInfo[id][fDiv4], mConnectionHandle, 8);
        cache_get_field_content(i, "Div5", FactionInfo[id][fDiv5], mConnectionHandle, 8);
        cache_get_field_content(i, "LockerX", szValue, mConnectionHandle); FactionInfo[id][fLockerPosition][0] = floatstr(szValue);
        cache_get_field_content(i, "LockerY", szValue, mConnectionHandle); FactionInfo[id][fLockerPosition][1] = floatstr(szValue);
        cache_get_field_content(i, "LockerZ", szValue, mConnectionHandle); FactionInfo[id][fLockerPosition][2] = floatstr(szValue);
        cache_get_field_content(i, "LockerMats", szValue, mConnectionHandle); FactionInfo[id][fMats] = strval(szValue);
...
{
	mysql_function_query(mConnectionHandle, "SELECT * FROM `factions`", true, "MySQL_Faction_Load", "i", mConnectionHandle);
}
...
public MySQL_Gates_Load(connectionHandle)
{
    ...
    {
        cache_get_field_content(i, "Id", szValue, mConnectionHandle); id = strval(szValue);
        cache_get_field_content(i, "Model", szValue, mConnectionHandle); GateInfo[id][gModel] = strval(szValue);
        cache_get_field_content(i, "Speed", szValue, mConnectionHandle); GateInfo[id][gSpeed] = strval(szValue);
        cache_get_field_content(i, "Password", GateInfo[id][gPassword], mConnectionHandle, 12);
        cache_get_field_content(i, "PosX", szValue, mConnectionHandle); GateInfo[id][gPosition][0] = floatstr(szValue);
        cache_get_field_content(i, "PosY", szValue, mConnectionHandle); GateInfo[id][gPosition][1] = floatstr(szValue);
        cache_get_field_content(i, "PosZ", szValue, mConnectionHandle); GateInfo[id][gPosition][2] = floatstr(szValue);
        cache_get_field_content(i, "PosRotationX", szValue, mConnectionHandle); GateInfo[id][gRotation][0] = floatstr(szValue);
        cache_get_field_content(i, "PosRotationY", szValue, mConnectionHandle); GateInfo[id][gRotation][1] = floatstr(szValue);
        cache_get_field_content(i, "PosRotationZ", szValue, mConnectionHandle); GateInfo[id][gRotation][2] = floatstr(szValue);
        cache_get_field_content(i, "DestinationX", szValue, mConnectionHandle); GateInfo[id][gDestPosition][0] = floatstr(szValue);
        cache_get_field_content(i, "DestinationY", szValue, mConnectionHandle); GateInfo[id][gDestPosition][1] = floatstr(szValue);
        cache_get_field_content(i, "DestinationZ", szValue, mConnectionHandle); GateInfo[id][gDestPosition][2] = floatstr(szValue);
        cache_get_field_content(i, "DestRotationX", szValue, mConnectionHandle); GateInfo[id][gDestRotation][0] = floatstr(szValue);
        cache_get_field_content(i, "DestRotationY", szValue, mConnectionHandle); GateInfo[id][gDestRotation][1] = floatstr(szValue);
        cache_get_field_content(i, "DestRotationZ", szValue, mConnectionHandle); GateInfo[id][gDestRotation][2] = floatstr(szValue);
...
public MySQL_Vehicle_Load(connectionHandle)
{
    new
        rows,
		fields,
    ...
    {
        cache_get_field_content(i, "Id", szValue); id = strval(szValue);
        cache_get_field_content(i, "Model", szValue); DynamicVehicleInfo[id][dvModel] = strval(szValue);
        cache_get_field_content(i, "PosX", szValue); DynamicVehicleInfo[id][dvPosition][0] = floatstr(szValue);
        cache_get_field_content(i, "PosY", szValue); DynamicVehicleInfo[id][dvPosition][1] = floatstr(szValue);
        cache_get_field_content(i, "PosZ", szValue); DynamicVehicleInfo[id][dvPosition][2] = floatstr(szValue);
        cache_get_field_content(i, "Angle", szValue); DynamicVehicleInfo[id][dvFacingAngle] = floatstr(szValue);
        cache_get_field_content(i, "Color1", szValue); DynamicVehicleInfo[id][dvColor][0] = strval(szValue);
        cache_get_field_content(i, "Color2", szValue); DynamicVehicleInfo[id][dvColor][1] = strval(szValue);
        cache_get_field_content(i, "Type", szValue); DynamicVehicleInfo[id][dvType] = strval(szValue);
        cache_get_field_content(i, "Faction", szValue); DynamicVehicleInfo[id][dvFaction] = strval(szValue);
...
{
	mysql_function_query(mConnectionHandle, "SELECT * FROM `vehicles`", true, "MySQL_Vehicle_Load", "i", mConnectionHandle);
}
...
//	cache_get_field_content(0, "IpAddress", PlayerInfo[playerid][pIpAddress], connectionHandle, 16);
    ...
    cache_get_field_content(0, "NewbMute", szValue, connectionHandle); PlayerInfo[playerid][pNewbMute] = strval(szValue);//
    ...
    cache_get_field_content(0, "OocJail", szValue, connectionHandle); PlayerInfo[playerid][pOocJail] = strval(szValue);//
    cache_get_field_content(0, "WantedLevel", szValue, connectionHandle); PlayerInfo[playerid][pWantedLevel] = strval(szValue);//
    cache_get_field_content(0, "Wounded", szValue, connectionHandle); PlayerInfo[playerid][pWounded] = strval(szValue);//
    ...
    cache_get_field_content(0, "VIP", szValue, connectionHandle); PlayerInfo[playerid][pVIP] = strval(szValue);//
    ...
    cache_get_field_content(0, "ContractReason", PlayerInfo[playerid][pContractReason], connectionHandle, 32);
    cache_get_field_content(0, "ContractBy", PlayerInfo[playerid][pContractBy], connectionHandle, MAX_PLAYER_NAME);
    cache_get_field_content(0, "Referer", PlayerInfo[playerid][pReferer], connectionHandle, MAX_PLAYER_NAME);
    ...
    for(new i=0; i < MAX_PLAYEROWNED_VEHICLES; i++)
    {
    	...
    	cache_get_field_content(0, szValue, szValue, connectionHandle); PlayerVehicleInfo[playerid][i][pvHealth] = floatstr(szValue);//
    	...
    	cache_get_field_content(0, szValue, szValue, connectionHandle, 128); split(szValue, mods, ',');
...
CMD:park(playerid, params[])
{
    ...
    for(new i=0; i < MAX_PLAYEROWNED_VEHICLES; i++)
    {
        if(GetPlayerVehicleID(playerid) == PlayerVehicleInfo[playerid][i][pvHandleId])
		{
			if(GetVehicleHealthEx(GetPlayerVehicleID(playerid)) < 800) return SendClientMessageEx(playerid, COLOR_GREY, "Your vehicle cannot be parked because it is too damaged to park it.");
...
public SavePlayerAccount(playerid, connectionHandle)
{
	...
	if(IsPlayerTabbed(playerid) == false && GetPlayerState(playerid) != PLAYER_STATE_WASTED && GetPlayerState(playerid) != PLAYER_STATE_SPECTATING && GetPVarType(playerid, "pSafeMode") == 0)
	{
		GetPlayerHealth(playerid, PlayerInfo[playerid][pHealth]);
		GetPlayerArmour(playerid, PlayerInfo[playerid][pArmour]);
	}
	...
	format(szQuery, sizeof(szQuery),"%s`Referer`='%s',\
		...
		`Accent`='%d',\
		`CompletedHits`='%d',\
		`Reports`='%d'", szQuery, PlayerInfo[playerid][pReferer], PlayerInfo[playerid][pDivision], PlayerInfo[playerid][pAccent], PlayerInfo[playerid][pCompletedHits], PlayerInfo[playerid][pReports]);
...
stock InitiatePaycheck()
{
	...
	foreach(new i: Player)
	{
	    ...
	    {
			new
				newCash = PlayerInfo[i][pPaycheck],
				safeTax = floatround((newCash / 100) * TAX_PERCENTAGE);

			SendClientMessageEx(i, COLOR_ORANGE, "------------------------------------------");
	        SendClientMessageEx(i, COLOR_ORANGE, "Paycheck Information");
	   		format(string, sizeof(string), "Account Name: %s, Paycheck: $%i", GetPlayerNameEx(i), newCash);
	        SendClientMessageEx(i, COLOR_WHITE, string);
	        PlayerInfo[i][pBankBalance] += newCash;
	   		format(string, sizeof(string), "Old Balance: $%i, New Balance: $%i", (PlayerInfo[i][pBankBalance] - newCash), PlayerInfo[i][pBankBalance]);
	        SendClientMessageEx(i, COLOR_WHITE, string);
	        PlayerInfo[i][pBankBalance] -= safeTax;
	   		format(string, sizeof(string), "Income Tax: %i percent, ($%i)", TAX_PERCENTAGE, safeTax);
            ...
            {
				format(string, sizeof(string), "[Paycheck]: %s has received $%d with a tax of $%d and $%d in his bank account.", PlayerInfo[i][pUsername], newCash, safeTax, PlayerInfo[i][pBankBalance]);
				Log("logs/payday.log", string);
			}
			printf("[Paycheck]: %s has received $%d with a tax of $%d and $%d in his bank account.", PlayerInfo[i][pUsername], newCash, safeTax, PlayerInfo[i][pBankBalance]);
            ...
            if(PlayerInfo[i][pExperience] >= (PlayerInfo[i][pLevel] + 1) * 5 && PlayerInfo[i][pLevel] < 50)
            {
				...
				SetPlayerScore(i, PlayerInfo[i][pLevel]);
			 	format(string, sizeof(string), "You are now level %d, you have have gained 2 experience points.", PlayerInfo[i][pLevel]);
				SendClientMessageEx(i, COLOR_WHITE, string);
...
stock GetCAdminRank(i)
{
	...
	{
	    case 0: { format(tmpString, sizeof(tmpString), "Level %i Player", PlayerInfo[i][pLevel]); }
	    case 1: { format(tmpString, sizeof(tmpString), "{FFFF00}Server Moderator:{FFFFFF}"); }
...
{
	if(GetPVarInt(playerid, "pGodMode") == 1) return 0;
...
{
	if(!strcmp(BusinessInfo[b][bOwner], PlayerInfo[playerid][pUsername], true)) {
		SendClientMessageEx(playerid, COLOR_WHITE, "The items were {00B30F}FREE{FFFFFF} because you are the owner of the business.");
	} else {
	    new string[128];
		GivePlayerCash(playerid, -amount);
	 	...
	 	#pragma unused x
	  	#pragma unused y
	   	if(z < -0.8)
	   	{
	   	    ...
	   	    {
				format(string, sizeof(string), "{FF6347}AdmWarning:{FFFF00} %s was just kicked from the server for having s0beit installed.", PlayerInfo[playerid][pUsername]);
				AdminAlert(string, ALERT_ADMINS);
		   	    KickEx(playerid);
		   	    return 0;
	   	    }
